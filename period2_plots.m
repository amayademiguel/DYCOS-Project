...
[tVr,  Vr]   = pull_signal(S,'Vr');      % regulator output
[tI,   Iabc] = pull_signal(S,'Iabc');    % 3-phase stator current
[tVf,  Vf]   = pull_signal(S,'Vf');      % control input u = field voltage
...
xlabel('Time [s]');
ylabel('V_t [pu]');
title('Terminal voltage','FontSize',fsTitle);
...
xlabel('Time [s]');
ylabel('\delta [deg]');
title('Rotor angle','FontSize',fsTitle);
legend('Location','best','FontSize',fsLegend);
nice_time_axis_full(tDel);
nice_y_axis(del);
savePNG('delta_scenario');
...
xlabel('Time [s]');
ylabel('\omega [pu]');
title('Rotor speed','FontSize',fsTitle);
legend('Location','best','FontSize',fsLegend);
nice_time_axis_full(tWm);
nice_y_axis(wm);
savePNG('omega_scenario');
...
xlabel('Time [s]');
ylabel('V_R [pu]');
title('Regulator output','FontSize',fsTitle);
legend('Location','best','FontSize',fsLegend);
...
xlabel('Time [s]');
ylabel('I [pu]');
title('Stator current magnitude','FontSize',fsTitle);
legend('Location','best','FontSize',fsLegend);
nice_time_axis_full(tI);
nice_y_axis(Imag);
savePNG('current_scenario');
...
xlabel('Time [s]');
ylabel('u [pu]');
title('Control input u = V_f','FontSize',fsTitle);
legend('Location','best','FontSize',fsLegend);
